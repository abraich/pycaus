...
class Simulation:
    ...
    def simule(self):
        ...
        pi_f = sigmoid(self.X.dot(self.beta) + self.coef_tt * tt )
        pi_cf = sigmoid(self.X.dot(self.beta) + self.coef_tt * (1-tt))
        Y_f = binomial(1, pi_f)
        ...
        self.y_0_prec = int((data_sim["Y_0"].mean() * 100))
        self.y_1_prec = int((data_sim["Y_1"].mean() * 100))
        
        # Wasserstein distances
        print("WD = ", wd)
        print(f"tt = 1 : {self.perc_treatement} % ")
        print(f"Y_0 = 1 : {self.y_0_prec} % ")
        print(f"Y_1 = 1 : {self.y_1_prec} % ")
        print(f"KLD = {KLD}")
        self.wd = wd
        self.kld = KLD
