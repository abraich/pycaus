...
def load_tcga(path="./final_data_1"):
    df = pd.read_csv(path + ".csv")
    df = df.rename(columns={
        'radiation_therapy': 'tt',
        'vital_status': 'event',
        'years_to_birth': 'T_f'
    })

    df['tt'] = df['tt'].map(dict(yes=1, no=0))
    encoder = ce.OneHotEncoder(cols='pathologic_stage',
                               handle_unknown='return_nan',
                               return_df=True,
                               use_cat_names=True)
    tt = df['tt']
    df = df.drop(['tt', 'T_f', 'event'], 1)
...
def load_metabric(path="./metabric"):
    df = pd.read_csv(path + ".csv")
    df = df.rename(columns={
        'x5': 'tt',
        'duration': 'T_f'
